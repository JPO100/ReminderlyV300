...
In edit mode, the effect computes effective toggle states before calling `computeAutoApplyResult`. For each parsed token, if that token's `category:lowercasedText` key is absent from the initial token set (captured on mount), the corresponding category's toggle is overridden to `false` for the purpose of that auto-apply evaluation. This allows new tokens to auto-apply while leaving unchanged tokens inert. See "Auto-apply in edit mode" above for the full description.
...
| Initial toggles | All off | Derived from saved reminder fields (date on if scheduled, time on if time exists, repeats on if repeatRule exists) |
...
The data model does not support time without date. This state cannot be constructed via the UI because:

- The only path to turn time on (manual toggle) auto-enables date.
- The only path to turn date off (manual toggle) cascades time off.
- NLC token application always sets date when setting time (compound tokens imply today; standalone time tokens enable date if not already on).

Behaviour for corrupted or legacy state where time exists without date is undefined and not explicitly handled.
