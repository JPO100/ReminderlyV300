...
function Frame1({ animationKey, filtersMenuVariant, isListsEnabled }: { animationKey: number; filtersMenuVariant: FiltersMenuVariant; isListsEnabled: boolean }) {
  if (isListsEnabled) {
    return (
...
function NusBlank({ animationKey, filtersMenuVariant, isListsEnabled }: { animationKey: number; filtersMenuVariant: FiltersMenuVariant; isListsEnabled: boolean }) {
  return (
    <div className={`absolute bg-[#4784f8] content-stretch flex flex-col h-[615px] items-center left-0 rounded-tl-[30px] rounded-tr-[30px] top-0 w-full ${isListsEnabled ? "justify-start gap-0" : "justify-between gap-[4px]"}`} data-name="NUS - Blank">
      ...
      <Frame1 animationKey={animationKey} filtersMenuVariant={filtersMenuVariant} isListsEnabled={isListsEnabled} />
    </div>
  );
}
    ...
    <div className="h-[361px] relative shrink-0 w-full max-w-[308px] [@media(max-height:570px)]:scale-[0.7] [@media(max-height:570px)]:origin-top [@media(max-height:570px)]:-mb-[108px]" data-name="Reminder colours">
      <div className="bg-[#1c2c42] h-full w-full rounded-tl-[40px] rounded-tr-[40px]" style={{ paddingTop: '14px', paddingLeft: '14px', paddingRight: '14px', boxSizing: 'border-box' }}>
        <div className="bg-[#4784f8] overflow-clip relative rounded-tl-[26px] rounded-tr-[26px] h-[calc(100%+2px)] w-full">
          <NusBlank animationKey={animationKey} filtersMenuVariant={filtersMenuVariant} isListsEnabled={isListsEnabled} />
        </div>
      </div>
    </div>
  );
...
export default function OnboardingPage2Content({ filtersMenuVariant, isListsEnabled = false, settingsMenuEnabled = true }: { filtersMenuVariant: FiltersMenuVariant; isListsEnabled?: boolean; settingsMenuEnabled?: boolean }) {
